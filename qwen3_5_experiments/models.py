...
from .config import JumpReluSaeConfig
...
class JumpReluResidualSae(nn.Module):
    """JumpReLU SAE trained on residuals of the base Qwen SAE.

    This is the SECONDARY SAE that learns patterns in the residual
    (hidden - base_SAE_reconstruction).
    """
...
def load_residual_sae(path: Path | str, device: str | torch.device = "cpu"):
    ...
    raise ValueError(f"Unknown SAE architecture: {cfg_dict.get('architecture')}")