...
from gemma3_experiments.vendor_sae import sae as sae_module
from gemma3_experiments.vendor_sae import utils as sae_utils
...
from .normalization import ResidualStats, save_residual_stats
...
def _initialize_decoder_bias(
    ...
    sae.b_dec.data.copy_(geometric_median(init_points))


def _disable_triton_if_needed(device: str) -> None:
    if device.startswith("cuda"):
        return
    sae_utils.decoder_impl = sae_utils.eager_decode
    sae_module.decoder_impl = sae_utils.eager_decode


def _save_checkpoint(sae: Sae, path: Path, stats: ResidualStats | None) -> None:
    sae.save_to_disk(path)
    if stats is not None:
        save_residual_stats(path, stats)


def _parameter_stats(sae: Sae) -> dict[str, float]:
    ...
    with torch.no_grad():
        ...
        if getattr(sae, "W_dec", None) is not None:
...
def _grad_is_finite(sae: Sae) -> bool:
    return all(param.grad is None or torch.isfinite(param.grad).all() for param in sae.parameters())


def _params_are_finite(sae: Sae) -> bool:
    return all(torch.isfinite(param).all() for param in sae.parameters())


def _make_sae(cfg: ResidualSaeTrainConfig, d_in: int, device: str):
    if cfg.sae_architecture == "topk":
        return Sae(d_in=d_in, cfg=cfg.sae, device=device, dtype=torch.float32)
    if cfg.sae_architecture == "jump_relu":
        return JumpReluResidualSae(d_in=d_in, cfg=cfg.jump_relu, device=device, dtype=torch.float32)
    raise ValueError(f"Unsupported SAE architecture: {cfg.sae_architecture!r}")
...
def _write_failure_report(
    ...
    *,
    ...
    with (output_dir / "failure_report.json").open("w") as fd:
...
def train_residual_sae(cfg: ResidualSaeTrainConfig) -> dict[str, str | int | float]:
    ...
    _disable_triton_if_needed(device)

    logger.info(
        "Starting residual SAE training: cache_dir=%s, output_dir=%s, d_in=%d, n_shards=%d, "
        "train_shards=%d, val_shards=%d, batch_size=%d, lr=%s, weight_decay=%s, lr_warmup_steps=%d, "
        "grad_clip_norm=%s, max_epochs=%d, max_steps=%s, architecture=%s, topk_expansion_factor=%s, k=%s, "
    ...
    sae = _make_sae(cfg, d_in=d_in, device=device)
    logger.info(
        "Initializing decoder bias from geometric median of first %d tokens",
    ...
    logger.info("Decoder bias initialized. SAE has %d latents.", sae.num_latents if hasattr(sae, "num_latents") else -1)
        ...
        for batch, shard_name in _iter_residual_batches(
            ...
            if not torch.isfinite(loss):
                ...
                _write_failure_report(
                    ...
                    epoch=epoch,
                    shard_name=shard_name,
                    reason="non-finite loss before backward",
                    grad_norm=None,
                    best_loss=best_loss,
                    best_step=best_step,
                    stats=_parameter_stats(sae),
                )
            ...
            if not math.isfinite(grad_norm_val) or not _grad_is_finite(sae):
                ...
                _write_failure_report(
                    ...
                    epoch=epoch,
                    shard_name=shard_name,
                    reason="non-finite gradients before optimizer step",
